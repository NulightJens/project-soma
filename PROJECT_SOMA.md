...
### 2026-04-23 (late night) — Usable loop lands (CLI + handlers + PM2 jobs-worker)
- **Reorder-for-usable path chosen** over HANDOFF's architectural-first order. Shell handler (HITL-gated per CLAUDE.md §3 — RCE surface) pushed behind CLI + trivial handlers + daemon integration so the queue → worker → result loop becomes exercisable by an operator end-to-end before any RCE-adjacent work lands.
- **`src/cli/jobs.ts` — operator CLI.** 7 subcommands (~360 LOC): `submit / list / get / cancel / retry / stats / work`. Uses the existing `src/cli/` commander pattern (registered in `src/cli/index.ts`). Trusted-submitter role per ADR-014: `--trusted` flag flips `{allowProtectedSubmit: true}` for the protected-names gate. Output follows ADR-014 in practice — every subcommand defaults to a plain-language summary and switches to raw JSON only on `--json`.
- **`src/cli/job-handlers.ts` — built-in handlers.** Minimal set exercised by the CLI's `--handlers` flag: `echo` (returns input + attempt), `noop` (empty), `sleep` (ms-based delay, cooperative-abort via `ctx.signal`). Ships so the full queue → worker → result loop can be verified without shell or API surface.
- **`src/cli/ecosystem.ts` — PM2 wired.** `cortextos ecosystem` now emits a `cortextos-jobs-worker` app alongside the daemon. Env overrides: `SOMA_MINIONS_DB`, `SOMA_WORKER_QUEUE`, `SOMA_WORKER_CONCURRENCY`, `SOMA_WORKER_HANDLERS`. Defaults: `$CTX_ROOT/minions.db`, queue `default`, concurrency 1, handlers `echo,noop,sleep`. Live PM2 doesn't pick up the new entry until the user re-runs `cortextos ecosystem && pm2 start ecosystem.config.js` — deliberately not triggered from this session (user-state-affecting action).
- **`tsup.config.ts` onSuccess hook.** Engine DDL (`schema.sql`) wasn't being copied to `dist/` under tsup's CJS bundle, so the built CLI crashed on SQLite engine open (`__dirname` resolves to `dist/` in the bundle). Added `onSuccess: 'cp src/minions/schema.sql dist/schema.sql'` so the schema travels with the bundle.
- **End-to-end verified against a temp DB:** submit an echo job → list (shows `waiting`) → launch `jobs work` → worker claims + runs + completes → `jobs get` shows `completed` with `{"echoed":{"msg":"hello from cli"},"attempt":1}`. First moment the system is actually usable by an operator.
...
### 2026-04-23 (late night) — SIGKILL-rescue regression (real subprocess)
- **`tests/cli-jobs-sigkill-rescue.test.ts`** — one focused regression case that complements the in-process stall-rescue smoke (`tests/minions-worker.test.ts`). The in-process version advances an injected clock and calls `handleStalled()` directly; this one spawns a real `node dist/cli.js jobs work` child, waits for it to claim a long `sleep` job, SIGKILLs it (no cooperative shutdown path), then starts a second worker with an aggressive stall-sweep interval and confirms the orphaned lock is requeued (stalled_counter goes from 0 → 1). Proves: token-fence + stall sweep survive OS-level process death end-to-end, not just unit-level.
- **Gracefully skips if `dist/cli.js` is missing** so a fresh clone doesn't red-light the suite before `npm run build` has run.
- **Runtime ~3.5s** — within vitest's default timeout; tagged with 60s per-test cap to absorb the mandatory 2.5s sleep past lock_until. The full fast suite is 91 tests in ~400ms; this single regression adds ~3s.
- **92/92 Minions+CLI+regression tests pass.** `tsc --noEmit` clean. Phase 1 at ~92% — the next two slots (shell handler, unified runner) both need HITL review per CLAUDE.md §3 (RCE surface) + §6 (OAuth / API-key handling), which is the natural session pause point.

### 2026-04-23 (late night) — Display rebrand to Project SOMA (ADR-015)
...
### 2026-04-23 (late night) — Shell handler port lands (HITL-approved)
- **`src/minions/handlers/shell.ts` ported verbatim** (311 LOC) from gbrain's handler. Only code deviation: `.ts` → `.js` import path per repo `moduleResolution: bundler` convention (annotated `// SOMA:`). Preserves every capability:
  - `cmd | argv` validation (exactly one, strings only, throws `UnrecoverableError` — not retry-worthy).
  - Required absolute-path `cwd` (trust boundary; operator chooses what the script can read).
  - Env allowlist `['PATH', 'HOME', 'USER', 'LANG', 'TZ', 'NODE_ENV']` merged with caller `job.data.env` — prevents `$OPENAI_API_KEY` leak into user-authored scripts.
  - Absolute `/bin/sh -c ...` (not `sh`) so a poisoned caller `PATH` can't redirect to a different shell binary.
  - UTF-8-safe `TailBuffer` with `[truncated N bytes]` marker (64KB stdout / 16KB stderr tails).
  - **Kill ladder:** subscribes to BOTH `ctx.signal` (timeout/cancel/lock-loss) AND `ctx.shutdownSignal` (worker SIGTERM/SIGINT). Either → SIGTERM → 5s grace → SIGKILL. Non-shell handlers ignore `shutdownSignal` so deploy restarts don't interrupt them mid-flight.
  - Signal-exit mapping: `SIGTERM`→143, `SIGKILL`→137. Non-zero exit → thrown `Error('exit N: ...')`. Abort-triggered kill → thrown `Error('aborted: ...')`.
- **Env-gate rename:** `GBRAIN_ALLOW_SHELL_JOBS` → `SOMA_ALLOW_SHELL_JOBS`. Keeps the dual-gate security: the env var gates worker-side handler registration; the protected-names gate (`src/minions/protected-names.ts`) blocks untrusted submission at the queue boundary.
- **Registration split.** `BUILTIN_HANDLERS` stays the always-on core set (`echo / noop / sleep`). New `resolveBuiltinHandlers()` returns the env-gated superset — adds `shell` iff `SOMA_ALLOW_SHELL_JOBS === '1'`. CLI `--handlers` resolution in `src/cli/jobs.ts` switched to the latter; a helpful "set SOMA_ALLOW_SHELL_JOBS=1" hint fires when an operator passes `--handlers shell` with the gate off. Existing test asserting `BUILTIN_HANDLERS` keys unchanged stays green.
- **PM2 ecosystem default stays safe:** `SOMA_WORKER_HANDLERS = echo,noop,sleep`. Shell must be added explicitly alongside `SOMA_ALLOW_SHELL_JOBS=1`; docstring in `src/cli/ecosystem.ts` updated to spell this out.
- **Test coverage:** `tests/minions-shell-handler.test.ts` — 15 vitest cases driving `shellHandler` directly with a stub `MinionJobContext` (fast, no DB round-trip). Validation × 7 (missing/both cmd-argv, non-string argv, missing/relative cwd, non-object env, non-string env value). Execution × 6 (cmd happy path, argv happy path, non-zero exit, env-allowlist blocks `process.env` secrets, caller `env` override reaches child, stdout truncation with `[truncated N bytes]` marker). Abort × 2 (`ctx.signal` + `ctx.shutdownSignal` each trigger kill ladder + throw `aborted:` Error). Plus 3 env-gate cases in `tests/cli-job-handlers.test.ts` — default excludes shell, `='1'` includes it, any other value ignored.
- **110/110 Minions+CLI+shell-handler tests pass** (up from 92: +15 shell handler + 3 env gate). `tsc --noEmit` clean repo-wide + dashboard.
- **Phase 1 at ~95%.** Remaining: unified runner (HITL-gated per CLAUDE.md §6 for OAuth/API-key handling) and the dashboard submit-UI intent-parser slot.

### 2026-04-24 — Unified runner + subscription engine lands (ADR-008 default path)
- **HITL discussion resolved 5 defaults** before coding: (1) subscription engine defaults to Claude CLI OAuth (Keychain + `CLAUDE_CODE_OAUTH_TOKEN` fallback); (2) env allowlist matches `src/pty/agent-pty.ts` keepVars + `CLAUDE_CODE_OAUTH_TOKEN`; (3) transcript reuses `minion_jobs.stacktrace` via `ctx.log` — no schema change this commit; (4) env gate `SOMA_ALLOW_SUBAGENT_JOBS=1` mirrors shell's pattern; (5) API-engine body deferred to a follow-up slot because it needs new schema + Anthropic SDK + tool registry. User green-lit all five.
- **Architectural seam — open engine registry.** Split into three files to satisfy ESM module semantics:
  - `src/minions/handlers/registry.ts` — pure registry (leaf module). `registerEngine` / `getEngine` / `listEngines` / `resetRegistryForTests` + all shared types (`RunnerEngine`, `RunnerResult`, `RunnerTokens`, `RunnerToolCall`, `RunnerEngineParams`). First attempt put the registry in `runner.ts`, which caused TDZ errors when engines imported from there and self-registered at load time (ESM hoists `import` statements so the engine's `registerEngine(...)` call fires before `runner.ts`'s top-level `const REGISTRY = new Map()` initialiser runs). The split breaks the cycle: engines → registry (one-way), runner → registry + engines (one-way).
  - `src/minions/handlers/runner.ts` — thin dispatcher. Validates `data.prompt`, picks engine via `data.engine` → `SOMA_DEFAULT_ENGINE` env → `'subscription'` default (ADR-008), hands off to `engine.run(ctx, params)`. Re-exports the registry API so downstream code has one stop (`import { runnerHandler, registerEngine } from '../handlers/runner.js'`). Side-effect imports `./engines/subscription.js` + `./engines/api.js` at module bottom.
  - Open for Hermes: adding a third engine is one file under `engines/` with a `registerEngine(...)` call at module top-level. No changes to `runner.ts` or downstream callers.
- **Subscription engine** (`src/minions/handlers/engines/subscription.ts`, ~350 LOC SOMA-original). Adapts gstack's `test/helpers/session-runner.ts` NDJSON pattern for the Minions worker contract:
  - `spawn('claude', ['-p', '--output-format', 'stream-json', '--verbose', '--dangerously-skip-permissions', '--max-turns', N, '--model', M, ...])` via Node's `child_process.spawn`. Prompt piped via stdin (no shell escaping). Env allowlist matches `src/pty/agent-pty.ts` keepVars plus `CLAUDE_CODE_OAUTH_TOKEN`.
  - **Pure `ingestNDJSONLine(acc, line)`** folds events into a `TranscriptAccumulator` (transcript array + tool_calls + assistant_turns + tokens + result event). Testable without a subprocess. Skips empty/malformed lines. Token usage (incl. `cache_read_input_tokens` + `cache_creation_input_tokens`) accumulated per-turn and overridden by the authoritative `result` event at close.
  - **Per-turn `ctx.log` emission** — on every assistant event that crosses the accumulator's turn counter, emit a `{type:'llm_turn', model, tokens_in, tokens_out, ts}` TranscriptEntry; per new tool_use, emit `{type:'tool_call', tool, args_size, result_size: 0, ts}`. Dashboard sees progress in real time via `minion_jobs.stacktrace` — no schema change.
  - **Running `ctx.updateTokens`** per chunk so the job row reflects mid-flight totals, not just the final count.
  - **Kill ladder mirrored from shell handler** — subscribes to both `ctx.signal` (timeout/cancel/lock-loss) and `ctx.shutdownSignal` (worker SIGTERM/SIGINT). Either → SIGTERM → 5s grace → SIGKILL. Abort-triggered kill throws `Error('aborted: ...')` so the worker classifies retry vs dead correctly.
  - **Exit-reason classification** from the `result` event's `subtype` when present (success / error_max_turns / ...), falling back to exit-code interpretation. Non-success exit without a result line throws (retryable at the worker).
  - Factory pattern `makeSubscriptionEngine({ binary, envOverride })` lets tests point at a fake `claude` shell script; the production engine auto-registers via `registerEngine(makeSubscriptionEngine())` at module load.
- **API engine stub** (`src/minions/handlers/engines/api.ts`, 30 LOC). Registers the name `'api'` so `data.engine='api'` routes deterministically and throws `UnrecoverableError` with a clear pointer to the subscription engine + the follow-up slot. Keeps the engine-selection seam live without pretending the implementation ships.
- **CLI wiring** — `src/cli/job-handlers.ts`'s `resolveBuiltinHandlers()` gained `subagent` + `subagent_aggregator` behind `SOMA_ALLOW_SUBAGENT_JOBS=1`. Both protected names share the same `runnerHandler`; aggregator's fan-in semantics (polling `child_done` inbox and feeding sibling outputs into the prompt) land with the API-engine port. `src/cli/jobs.ts`'s unknown-handler error hints at the matching env flag. `src/cli/ecosystem.ts` docstring updated to spell out the gate combo.
