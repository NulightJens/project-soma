...
> **Hosted version:** https://nulightjens.github.io/project-soma/ — same content rendered with Astro Starlight, plus AI-agent-friendly `/llms.txt`, `/llms-full.txt`, and `/llms-small.txt`. Goes live once the repo's **Settings → Pages → Source** is set to "GitHub Actions" (one-time setup). See [docs-site/README.md](../../docs-site/README.md) for build details.
