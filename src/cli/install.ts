...
export const installCommand = new Command('install')
  ...
  .action(async (options: { instance: string }) => {
    ...
    try {
      ...
      console.error('    Then run: npm install (in the SOMA directory)');
      process.exit(1);
    }
    ...
    // Register SOMA CLI globally so agent PTY sessions can find it
    console.log('Registering SOMA CLI globally...');
    ...
    if (linkResult.status === 0) {
      console.log('  ✓ SOMA registered globally (npm link)');
    } else {
      console.log('  ! npm link failed. Run manually: npm link (from the SOMA directory)');
