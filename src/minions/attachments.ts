 ...
 *
 * Ported verbatim from gbrain's `src/core/minions/attachments.ts`
 * (MIT © Garry Tan). Pure function — decoupled from the DB so it can be
 * unit-tested without an engine.
 *
 * The DB `UNIQUE (job_id, filename)` constraint (schema.sql) is the
 * authoritative duplicate fence; the in-memory `existingFilenames` check
 * just gives a faster, clearer error before the round-trip.
 *
 * SOMA adaptations (annotated inline):
 *   - `./types.ts` import → `./types.js` per repo moduleResolution: bundler.
 */
...
export function validateAttachment(
  ...
  if (
    ...
    return {
      ok: false,
      error: `filename contains invalid characters: ${JSON.stringify(filename)}`,
    };
  }
  ...
  // Strict base64: only A-Z a-z 0-9 + / and trailing =. Reject whitespace
  // and line breaks so callers normalize before sending (no silent
  // corruption).
